...
4. Select the site visibility:
    ...
    * **Private**: Only available to the site manager and any users added to the site.
    ...
    > **Note:** The visibility setting you select is displayed next to the site name when a user is in the site. See [Alfresco Share sites]({% link content-services/community/using/sites/index.md %}) for more information on site visibility settings.
...
2. Enter a search term such as a user name and click **Search**.
    ...
    > **Note:** If you don't see any users then try a different search term and check that your Alfresco administrator has [created users]({% link content-services/community/admin/users-groups.md %}).
...
4. Set the site role for each user, or use **Set all roles to** to assign the same role to all users. This controls what the user can do in the site.
    ...
    > **Note:** See [User roles and permissions]({% link content-services/community/using/permissions.md %}) for more about site roles.
...
5. Click **Add Users**.
    ...
    > **Note:** This feature is disabled if your installation doesn't support inviting new users. Talk to your system administrator about enabling this feature with the `notification.email.siteinvite` property. See [Outbound SMTP configuration properties]({% link content-services/community/config/email.md %}#outbound-smtp-configuration-properties) for more information.
